...
When the **"Issue Personal Info Credential"** button is clicked in the Django app, the following process occurs:
...
2. **Form Submission:**
   The form submits a POST request to a Django view (e.g., `/issue_personal_info/`).

   ```html
   <form method="post" action="{% url 'issue_personal_info' %}">
     <input type="checkbox" name="revocable" value="true"> Revocable
     <input type="checkbox" name="expiry" value="true"> Expiry
     <button type="submit">Issue Personal Info Credential</button>
   </form>
   ```

3. **Django View Handler:**
   The Django view reads the POST data and constructs the credential payload accordingly.

   ```python
   from django.views.decorators.csrf import csrf_exempt
   from django.http import JsonResponse
   from webapp.services import build_credentials_request
   from webapp.util import startIssuance

   @csrf_exempt
   def issue_personal_info(request):
     if request.method == "POST":
       revocable = request.POST.get("revocable") == "true"
       expiry = request.POST.get("expiry") == "true"
       # Build credential payload using helper
       payload = build_credentials_request("personalInformation")
       # Optionally add revocable/expiry to payload/metaData
       if payload and revocable:
         payload[0]["metaData"] = payload[0].get("metaData", {})
         payload[0]["metaData"]["revocable"] = True
       if payload and expiry:
         payload[0]["metaData"] = payload[0].get("metaData", {})
         payload[0]["metaData"]["expirationDate"] = "2027-09-01T00:00:00.000Z"
       # Call Affinidi CIS API to issue credential
       response = startIssuance(payload)
...
**Summary:**
- Button click triggers form POST to Django view
- View reads checkbox values and builds payload
- Credential is issued with revocable/expiry options if selected
- Result is shown to the user

Refer to your Django view, `webapp/services.py` for payload construction, and `webapp/util.py` for Affinidi CIS API integration.
